...
describe('P6 Platform Security Operations Controller Suite', () => {
  ...
  beforeEach(() => {
    mockSecOpsService = {
      ...
      getSecurityMetrics: jest.fn().mockResolvedValue({
        period: '24h',
        metrics: {
          loginSuccessCount: 150,
          loginFailureCount: 2,
          openIncidentsCount: 0,
        },
        anomaliesDetected: [],
      }),
      getSecurityTimeline: jest
        .fn()
        .mockResolvedValue([
    ...
    mockAlertsService = {
      listAlerts: jest
        .fn()
        .mockResolvedValue({ alerts: [], pagination: { total: 0 } }),
      runDetectionPass: jest
        .fn()
        .mockResolvedValue({ success: true, alertsCreated: 0, alerts: [] }),
      ...
      escalateAlertToIncident: jest.fn().mockResolvedValue({
        success: true,
        incident: { id: 'inc-1' },
        alertId: 'alert-1',
      }),
    ...
    mockEmergencyService = {
      forcePasswordReset: jest
        .fn()
        .mockResolvedValue({ success: true, message: 'Password reset forced' }),
    };
  ...
  describe('1. Controller Endpoints', () => {
    ...
    it('triggers threat detection via POST /alerts/detect', async () => {
      ...
      expect(res.success).toBe(true);
      expect(mockAlertsService.runDetectionPass).toHaveBeenCalledWith(
        'admin-1',
      );
    });

    it('forces password reset via POST /emergency/force-password-reset/:userId', async () => {
      const res = await controller.forcePasswordReset(
        'usr-1',
        { reason: 'Suspicious login' },
        { user: { id: 'admin-1' } },
      );
      expect(res.success).toBe(true);
      expect(mockEmergencyService.forcePasswordReset).toHaveBeenCalledWith(
        'usr-1',
        'Suspicious login',
        'admin-1',
      );
