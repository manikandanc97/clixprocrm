...
export class RazorpayAdapter implements IPaymentGatewayAdapter {
  ...
  async createOrder(params: CreateOrderParams): Promise<PaymentOrderResult> {
    const keyId =
      this.keyId || process.env.RAZORPAY_KEY_ID || 'rzp_test_clixpro';
    ...
    const isMock =
      ...
      (process.env.NODE_ENV === 'test' && !keyId.startsWith('rzp_test_TXR'));
    ...
    if (keySecret && !isMock && keySecret.length > 8) {
      try {
        ...
        if (response.ok) {
          ...
          this.logger.log(
            `[RAZORPAY ORDER CREATED] Order ID: ${data.id} | Amount: ${data.amount} ${data.currency}`,
          );
          return {
            provider: 'RAZORPAY',
            orderId: data.id,
          ...
          const errMsg =
            ...
            `Razorpay order creation failed with HTTP status ${response.status}`;
          this.logger.error(
            `[RAZORPAY ORDER FAILURE] (${response.status}): ${errMsg}`,
          );
    ...
    this.logger.log(
      `[RAZORPAY TEST MOCK ORDER GENERATED] Order ID: ${deterministicOrderId} | Amount: ${amount} ${params.currency || 'INR'}`,
    );
    return {
      provider: 'RAZORPAY',
  ...
  verifyPaymentSignature(params: VerifySignatureParams): boolean {
    const secret =
      params.secret || this.keySecret || process.env.RAZORPAY_KEY_SECRET;

    if (!secret || !params.orderId || !params.paymentId || !params.signature) {
      this.logger.warn(
        '[RAZORPAY SIGNATURE CHECK FAILED] Missing orderId, paymentId, signature, or secret',
      );
      return false;
    }
    ...
    try {
      ...
      this.logger.error(
        `Razorpay signature verification error: ${err.message}`,
      );
      return false;
    }
  }
  ...
  async verifyAndParseWebhook(
    ...
    if (secret) {
      ...
      try {
        ...
        this.logger.error(
          `Razorpay webhook signature verification failure: ${err.message}`,
        );
        return null;
      }
    }
    ...
    try {
      ...
      this.logger.error(
        `Failed to parse Razorpay webhook payload: ${parseErr.message}`,
      );
      return null;
    }
  }
