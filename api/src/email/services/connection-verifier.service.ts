...
export class ConnectionVerifierService {
  ...
  async assertSafeHost(host: string): Promise<void> {
    if (!host || typeof host !== 'string') {
      throw new BadRequestException(
        'Host is required and must be a valid string',
      );
    }
    ...
    if (
      ...
      throw new BadRequestException(
        `Connection refused: Host '${host}' is blocked for security`,
      );
    }

    if (net.isIPv4(trimmed) && isPrivateOrReservedIPv4(trimmed)) {
      throw new BadRequestException(
        `Connection refused: IPv4 address '${host}' is in a reserved network`,
      );
    }

    if (net.isIPv6(trimmed) && isPrivateOrReservedIPv6(trimmed)) {
      throw new BadRequestException(
        `Connection refused: IPv6 address '${host}' is in a reserved network`,
      );
    }
    ...
    await new Promise<void>((resolve, reject) => {
      dns.lookup(trimmed, { all: true }, (err, addresses) => {
        if (err || !addresses || addresses.length === 0) {
          return reject(
            new BadRequestException(`DNS resolution failed for host: ${host}`),
          );
        }
        for (const addr of addresses) {
          if (addr.family === 4 && isPrivateOrReservedIPv4(addr.address)) {
            return reject(
              new BadRequestException(
                `Connection refused: Host resolves to private IPv4 address`,
              ),
            );
          }
          if (addr.family === 6 && isPrivateOrReservedIPv6(addr.address)) {
            return reject(
              new BadRequestException(
                `Connection refused: Host resolves to private IPv6 address`,
              ),
            );
          }
        }
  ...
  async verifySmtp(
    ...
    try {
      ...
      this.logger.warn(
        `SMTP verification failed for ${params.host}:${params.port}: ${err?.message || err}`,
      );
      return {
        success: false,
        error: this.sanitizeErrorMessage(
          err?.message || 'SMTP connection or authentication failed',
        ),
      };
    }
  }
  ...
  async verifyImap(
    ...
    try {
      ...
      const isSecure =
        params.secure !== undefined ? params.secure : port === 993;
        ...
        (resolve) => {
          ...
          const timer = setTimeout(() => {
            if (!isResolved) {
              ...
              resolve({
                success: false,
                error: 'IMAP connection timed out after 10 seconds',
              });
            }
          }, 10000);
          ...
          const onData = (data: Buffer) => {
            ...
            if (buffer.includes('A01 OK')) {
              ...
              buffer.includes('A01 NO') ||
              buffer.includes('A01 BAD')
            ) {
              socket.write('A02 LOGOUT\r\n');
              safeResolve({
                success: false,
                error:
                  'IMAP authentication failed: Invalid credentials or rejected command',
              });
            }
          };
          ...
          try {
            ...
            socket.on('error', (err: any) => {
              safeResolve({
                success: false,
                error: this.sanitizeErrorMessage(
                  err?.message || 'IMAP socket connection error',
                ),
              });
            });
            ...
            safeResolve({
              success: false,
              error: this.sanitizeErrorMessage(
                connectErr?.message || 'Failed to initialize IMAP connection',
              ),
            });
          }
        },
      ...
      this.logger.warn(
        `IMAP verification failed for ${params.host}:${params.port}: ${err?.message || err}`,
      );
      return {
        success: false,
        error: this.sanitizeErrorMessage(
          err?.message || 'IMAP connection verification failed',
        ),
      };
    }
  }
